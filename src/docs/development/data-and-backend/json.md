...
어느 시점부터 웹 서버랑 통신하지 않거나 구조화된 데이터를 쉽게 보관하는 모바일 앱을
생각하기 어려워졌습니다. 네트워크와 연결된 앱을 제작할 때, 결국에는 제법 괜찮은 JSON 을
사용하게 되어있습니다.
...
{{site.alert.info}}
  **Terminology:** _인코딩_ 과 _직렬화_ 는 자료 구조를 문자열로 변환하는 서로 같은 일을 합니다. 
  _디코딩_ 과 _역직렬화_ 는 문자열을 자료 구조로 변환하는 정반대 과정을 거칩니다.
  하지만, _직렬화_ 는 일반적으로 자료 구조를 보다 읽기 쉬운 형태로 변환하는 과정을 가르킵니다.

  혼란을 피하기 위해, 이 문서에서는 전반적인 과정을 언급할 때 "직렬화"이라 합니다.
  그리고 특히 각각의 과정들을 언급할 때 "인코딩"과 "디코딩"이라 합니다.
{{site.alert.end}}
...
각각의 프로젝트들은 서로 다른 복잡도와 사용 사례에 직면합니다. 소규모의 PoC 프로젝트와
빠른 변화를 요구하는 프로토타입에게 코드 생성기를 이용하는 것은 지나칠 수 있습니다. 더 복잡한 JSON 모델을 다루는 앱은
수작업으로 인코딩을 하기에는 지루하고, 반복적이고, 자그마한 에러들이 많이 일어나게 될 수 있습니다.

### 소규모 프로젝트에는 수동 직렬화를 사용하세요

수동 JSON 디코딩은 `dart:convert` 에 탑재되어 있는 JSON 디코더를 사용하는 것을 가리킵니다.
...
만약 프로젝트에 JSON 모델이 많이 없고 개념을 빠르게 검증해 보고 싶을 때,
...
런타임 리플렉션은 [Tree shaking][]을 방해하는데, Dart가 꽤나 오랫동안 지원했던 기능입니다.
...
이 방법으로, _호출부_ 에서 `name` 과 `email` 필드에 대해 타입 안전성, 자동완성, 
컴파일 타임 오류가 가능하게 됩니다. 만약 필드를 `String`형 대신에 `int`형으로
취급한다던가, 그런 오타를 내면 런타임 오류 대신 컴파일이 조차 되지 않을 것입니다.
...
이 방법을 통해 호출 부에서는 JSON 직렬화를 전혀 걱정하지 않아도 됩니다. 하지만,
모델 클래스는 아직 직렬화에 대해 확실해야 합니다. 프로덕션 앱에서 직렬화가 제대로 동작하는지
보장되어야 합니다. 실제로는, `User.fromJson()` 과 `User.toJson()` 메서드 모두
...
직렬화 코드가 더 손수 작성되지 않고, 수동적으로 관리되지 않기에, 런타임 때 JSON 직렬화 오류의
위험을 최소화할 수 있습니다.

### 프로젝트에서 json_serializable 설정하기

프로젝트에 `json_serializable`를 포함시키기위해, 일반 의존성 한 개와 _개발 의존성_ 두 개가 필요합니다.
간단히 말하자면,  _개발 의존성_ 은 앱 소스코드에 포함되지 않고 오직 개발 환경에서만 사용되는 의존성입니다.
...
프로젝트의 루트 폴더에서 `flutter pub get` 를 실행하세요. (아니면 에디터에서 **Packages Get** 을
클릭하세요.) 이 과정을 통해 프로젝트에 필요한 새 의존성 들을 포함해줍니다.
...
/// 접근 가능하도록 해줍니다. 여기에 *.g.dart 형식이 들어갑니다.
/// * 에는 소스 파일의 이름이 들어갑니다.
...
class User {
  ...
  /// map 에서 새로운 User 객체를 생성하기 위해 필요한 팩토리 생성자입니다.
...
`flutter pub run build_runner build` 을 프로젝트 루트에서 실행하면, 
모델의 JSON 직렬화 코드를 생성할 수 있습니다. 이 명령은 소스 코드를 살펴보며 관련된 부분을 찾고,
필요한 직렬화 코드를 생성해내는 일회성 빌드입니다.

이 방법도 편리하지만, 모델 클래스에 변화가 있을 때 마다 빌드를 실행하지 않으면 더 좋을 것 같네요.
